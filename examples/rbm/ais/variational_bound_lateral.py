...
def p(v,h):
    ...
    #p_h = log((1+exp(dot(w0[:,0:-1],h[:,:])))).sum(axis=0) + (dot(w1[0:-1,-1],h[0:-1,:]))
    #print "log(p(h)): ", log(p_h), " log(p(v|h)): ", log(p_v_h), " log (p(v)) ",log(p_v_h*p_h)
    p_vh = dot(v.T,dot(w0,h)) + dot(h.T,dot(iw,h)) 
    return p_vh
...
def variational_bound_lateral(path)
### load mnist
...
### load weights
    w0=load(path+"weights-0.npy")
    iw=load(path+"iweights-0.npy")
...
if len(sys.argv)>1:
    path=sys.argv[1]
variational_bound_lateral(path)
