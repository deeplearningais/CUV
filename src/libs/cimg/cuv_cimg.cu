...
#include "cuv_cimg.hpp"

namespace cuv{
	namespace cimg{
		using namespace cimg_library;
		template<class V,class M,class I>
		void load(dense_matrix<V,M,host_memory_space,I>& m, const std::string& name, column_major ){
			CImg<V> img(name.c_str());
			dense_matrix<V,row_major,host_memory_space,I> m2(img.height(),img.width(),(V*) img.data(),true);
			cuv::copy(m.vec(),m2.vec());
		}
		template<class V,class M,class I>
		void load(dense_matrix<V,M,host_memory_space,I>& m, const std::string& name, row_major ){
			CImg<V> img(name.c_str());
			dense_matrix<V,row_major,host_memory_space,I> m2(img.width(),img.height(),(V*) img.data(),true);
			m = m2;
		}
		template<class V,class M,class I>
		...
		void show(const dense_matrix<V,M,host_memory_space,I>& m, const std::string& name ){
			if(IsSame<M,column_major>::Result::value){
					dense_matrix<const V,row_major,host_memory_space,I> m2(m.w(), m.h(),m.ptr(),true);
					show(m2, name);
			}else{
				typedef typename unconst<V>::type Vuc;
				CImg<Vuc> img(const_cast<Vuc*>(m.ptr()),m.w(),m.h());
...
#define LOAD_INST_FULL(V,M,I) \
		template void load<V,M,I>(dense_matrix<V,M,host_memory_space,I>&, const std::string&); \
		template void save<V,M,I>(dense_matrix<V,M,host_memory_space,I>&, const std::string&);
...
#define INST_FULL(V,M,I) \
		template void show<V,M,I>(const dense_matrix<V,M,host_memory_space,I>&, const std::string&);
#define INST_V(V) \
