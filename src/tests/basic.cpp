...
{
	dense_matrix<float,column_major,dev_memory_space> m(16,16);
	dense_matrix<float,column_major,dev_memory_space> m2(16,16,new dev_vector<float>(m.n(), m.ptr(), true));
}
...
BOOST_AUTO_TEST_CASE( create_host )
{
	...
	dense_matrix<float,column_major,host_memory_space> m2(16,16,new host_vector<float>(m.n(),m.ptr(),true));
}
