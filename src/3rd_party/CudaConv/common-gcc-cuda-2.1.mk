...
SRCDIR     ?= 
SDKDIR     ?= $(CUDA_SDK_PATH)
ROOTBINDIR ?= ./bin
BINDIR     ?= $(ROOTBINDIR)/$(OSLOWER)
ROOTOBJDIR ?= obj
SDK_LIBDIR     := $(SDKDIR)/lib
SDK_COMMONDIR  := $(SDKDIR)/common
...
# Includes
INCLUDES  += -I. -I$(CUDA_INSTALL_PATH)/include -I$(SDK_COMMONDIR)/inc
...
NVCCFLAGS := 
CXXFLAGS  := $(CXXWARN_FLAGS) -idirafter /usr/include/linux
CFLAGS    := $(CWARN_FLAGS)
LINKFLAGS := 
...
# Libs
LIB       ?= 
LIB       += -L$(CUDA_INSTALL_PATH)/lib -L$(SDK_LIBDIR) -L$(COMMONDIR)/lib/$(OSLOWER) 
ifeq ($(USEDRVAPI),1)
...
ifneq ($(STATIC_LIB),)
	TARGETDIR := $(SDK_LIBDIR)
	TARGET   := $(subst .a,$(LIBSUFFIX).a,$(SDK_LIBDIR)/$(STATIC_LIB))
	...
	LINKLINE  = $(LINK) $(LINKFLAGS) -o $(TARGET) $(OBJS) $(LIB)
endif
